    ...
    const fetchData = async () => {
      try {
        ...
        if (usertype === 'Sales Manager') {
          ...
          response = await axios.get(`http://localhost:8081/callLogs/${sales_person}`);
        }
  
        const formattedData = response.data.map((lead) => ({
          log_id: lead.log_id,
          date: lead.date && lead.date !== "0000-00-00" ? new Date(lead.date).toLocaleDateString("en-CA") : "",
          lead_name: lead.lead_name,
          lead_status: lead.lead_status,
          follow_up_date: lead.follow_up_date && lead.follow_up_date !== "0000-00-00" ? new Date(lead.follow_up_date).toLocaleDateString("en-CA") : "",
          note: lead.note,
